...
def tt_round(tt, max_tt_rank=None, epsilon=None, name='ttax_round'):
  ...
  for core_idx in range(num_dims - 1, 0, -1):
    ...
    rows = list(curr_core.shape)[0]
    if rows is None:
      rows = jnp.shape(curr_core)[0]
    if max_tt_rank[core_idx] == 1:
      ...
      try:
        ...
        # they are tensors (?) instead of values.
        min_dim = jnp.minimum(rows, columns)
        ranks[core_idx] = jnp.minimum(max_tt_rank[core_idx], min_dim)
    ...
    tt_cores[core_idx] = jnp.reshape(jnp.transpose(v), core_shape)
  ...
  def orthogonalize_tt_cores(tt, left_to_right=True,
                           name='ttax_orthogonalize_tt_cores'):
