...
def grad(func: Callable[[TTOrTTMat], float]) -> Callable[[TTOrTTMat], TangentVector]:
  ...
  """
  def _grad(x: TTOrTTMat) -> TangentVector:
    ...
    def augmented_func(d):
      x_projection = riemannian.deltas_to_tangent_space(d, left, right)
    ...
    return riemannian.deltas_to_tangent_space(deltas, left, right)
  return _grad


def hessian_vector_product(func: Callable[[TTOrTTMat], float]) -> Callable[[TTOrTTMat, TangentVector], TangentVector]:
  """P_x [d^2f/dx^2] P_x vector, i.e. Riemannian hessian by vector product.
    ...
    See also:
        `ttax.grad`
    """
  def _hess_by_vec(x: TTOrTTMat, vector: TangentVector) -> TangentVector:
    left = decompositions.orthogonalize_tt_cores(x)
    right = decompositions.orthogonalize_tt_cores(left, left_to_right=False)
    deltas = [right.tt_cores[0]]
    deltas += [jnp.zeros_like(cc) for cc in right.tt_cores[1:]]
    ...
    def augmented_outer_func(deltas_outer):
      ...
      def augmented_inner_func(deltas_inner):
        x_projection = riemannian.deltas_to_tangent_space(deltas_inner, left,
                                                          right)
        return func(x_projection)
